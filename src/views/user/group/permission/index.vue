<template>
  <div class="app-container">
    <el-row style="float:right;margin-bottom: 5px;margin-right:25px;"  v-if="value!=''">
    ...
    <el-table :data="permissionList" style="width: 100%;margin-top:30px;" border stripe>
      ...
      <el-table-column align="center"  label="Model name" width="220">
        <template slot-scope="scope">
          {{ scope.row.model }}
          <!-- <el-checkbox v-model="checkAll" @change="handleCheckAllChange($event,scope.row.permission)"></el-checkbox> -->
        </template>
      </el-table-column>
      ...
      <el-table-column header-align="center" label="Permission name">
        <template slot-scope="scope">
          <el-checkbox-group v-model="checkedbox">
            <el-checkbox style="width:300px" v-for="p in scope.row.permission" :label="p.id" :key="p.id">{{p.name}}</el-checkbox>
          </el-checkbox-group>
        </template>
      </el-table-column>
    ...
    getGroupPermission() {
      getGroupPermission(this.group_id)
        .then(response => {
          ...
          this.permissionList = response.all_permission 
